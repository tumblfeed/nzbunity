...
function Options() {
  ...
  const [currentDownloader, setCurrentDownloader] = useState(null);
  const addDownloader = () => {
  ...
  const saveDownloader = async (downloader) => {
    if (!options) return;
  ...
  const removeDownloader = async (downloader) => {
  ...
  const saveIndexer = async (name, indexer) => {
    if (!options) return;
