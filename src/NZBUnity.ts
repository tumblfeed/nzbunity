...
export class NZBUnity {
  ...
  async getProviderMatchRegex(): Promise<Array<RegExp>> {
    ...
    return matches.map(m => m.replace('//*.', '//*').replace(/\*/g, '.*')).map(m => new RegExp(`^${m}$`));
  }
  ...
  showNotification(id: string, title: string, message: string) {
    Util.storage.get('EnableNotifications').then(opts => {
  ...
  sendTabMessage(tab: chrome.tabs.Tab, name: string, data: any): Promise<any> {
    return tab
      ? Util.sendTabMessage(tab.id, { [`main.${name}`]: data }).catch(err => {})
  ...
  startTimer() {
    ...
    Util.storage.get('RefreshRate').then(opts => {
  ...
  async normalizeCategory(category: string | null | undefined): Promise<string> {
    const opts = await Util.storage.get([
      'DefaultCategory',
      'IgnoreCategories',
      'OverrideCategory',
      'SimplifyCategories',
      ...
      switch (k) {
        ...
        case 'newznab.enable':
          Util.storage
            .get(['ProviderNewznab'])
            .then(opts => {
              const providers = opts.ProviderNewznab.split(/\s*,\s*/).filter(i => {
                return i;
              });
        ...
        // Content Scripts
        case 'content.addUrl':
          this.addUrl(val.url, val).then(res => sendResponse(res && res.success));
          break;

        case 'content.addFile':
          this.addFile(val.filename, val.content, val).then(res => sendResponse(res && res.success));
          break;
        ...
        case 'popup.setMaxSpeed':
          if (this.nzbHost) {
            this.nzbHost.setMaxSpeed(val).then(() => {
              this.refresh();
              sendResponse(undefined);
            });
          }
        ...
        case 'options.onTab':
          this.optionsTab = val;
          this.sendOptionsMessage('onTab', true).then(res => sendResponse(res && res.success));
          break;
        ...
        case 'options.resetOptions':
          this.resetOptions().then(() => {
            this.sendOptionsMessage('resetOptions', true).then(res => sendResponse(res && res.success));
          });
          break;

        case 'options.profileNameChanged':
          Util.storage.get('ActiveProfile').then(opts => {
  ...
  handleCommand(command: chrome.commands.CommandEvent) {
    ...
    switch (String(command)) {
      case 'resume-queue':
        if (this.nzbHost) {
          this.nzbHost.resumeQueue().then(() => this.refresh());
        }
        break;

      case 'pause-queue':
        if (this.nzbHost) {
          this.nzbHost.pauseQueue().then(() => this.refresh());
        }
        break;
        ...
        if (this.nzbHost) {
          this.nzbHost
            ...
            .then((queue: NZBQueueResult) => {
              return queue.status.toLowerCase() === 'paused' ? this.nzbHost.resumeQueue() : this.nzbHost.pauseQueue();
            })
            .then(r => this.refresh());
        }
        break;
  ...
  handleStorageChanged(changes: { string: chrome.storage.StorageChange }, area: string) {
    this.debug(
      '[OptionsPage.handleStorageChanged] ',
      Object.keys(changes)
    ...
    if (changes['InterceptDownloads']) {
      if (changes['InterceptDownloads'].newValue) {
        this.enableIntercept();
        return Util.storage.get(['InterceptExclude']).then(opts => {
          this.interceptExclude = opts.InterceptExclude;
        });
      } else {
  ...
  resetOptions(): Promise<void> {
    return Util.storage.clear().then(() => this.initOptions());
  }
    ...
    return Util.storage
      ...
      .then(opts => {
        if (opts.Initialized) {
          ...
          return this.setOptionDefaults().then(() => Util.storage.set({ Initialized: true }));
        }
      })
      .then(() => {
        this.initProviders();
  ...
  initProviders(): Promise<{ [key: string]: NZBUnityProviderOptions }> {
    return Util.storage.get('Providers').then(opts => {
      const providers: { [key: string]: NZBUnityProviderOptions } = {};
  ...
  openProfilePage(): Promise<any> {
    return this.getActiveProfile().then(profile => {
      if (profile) {
        let url: string;
        ...
        if (profile.ProfileServerUrl) {
          ...
          url = profile.ProfileHost.replace(/\/?(api|jsonrpc)$/gi, '');
        } else {
          // Default to host
  ...
  async handleHeadersReceived(details: chrome.webRequest.WebResponseHeadersDetails) {
    ...
    const dispositionMatch = disposition && disposition.match(/^attachment;\s*filename="?(.*(\.nzb))"?$/i);
    if (allowed && details.method == 'GET' && (type === 'application/x-nzb' || dispositionMatch)) {
      // console.log('===================HEADERS=================');
      // console.log(`URL: ${url}`);
      ...
      this.addUrl(url, options).then(r => {
  ...
  profileTest(name: string): Promise<NZBResult> {
    return this.nzbHost ? this.nzbHost.test() : Promise.reject({ success: false, error: 'No connection to host' });
  }
    ...
    return Util.storage
      ...
      .then(opts => {
        ...
        if (!name || !profileNames.includes(name)) {
          ...
          name = profileNames.includes(opts.ActiveProfile) ? opts.ActiveProfile : profileNames[0];
        }
  ...
  getActiveProfile(): Promise<NZBUnityProfileOptions> {
    return Util.storage.get(['ActiveProfile', 'Profiles']).then(opts => opts.Profiles[opts.ActiveProfile]);
  }
  ...
  debug(...args: any[]) {
    Util.storage.get('Debug').then(opts => {
      if (opts.Debug) {
        const msg = args
          .map(arg => {
            switch (typeof arg) {
