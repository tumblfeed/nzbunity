class NZBUnityNzbfinder {
  ...
  // Site is currently being redesigned. The new version has slightly different url sctructures 
  // TODO: Remove after v4 launch
  public v3:boolean = false; 
    ...
    Util.storage.get(['Providers', 'ReplaceLinks'])
      .then((opts) => {
        ...
        if (enabled) {
          ...
          // TODO: Remove after v4 launch
          this.v3 = /v3/i.test($('meta[name="version"]').attr('content'));
  ...
  getNzbUrl(id:string):string {
    // TODO: Remove after v4 launch
    return this.v3
      ? `https://nzbfinder.ws/getnzb/${id}?i=${this.uid}&r=${this.apikey}`
      : `${window.location.origin}/getnzb?id=${id}&r=${this.apikey}`;
